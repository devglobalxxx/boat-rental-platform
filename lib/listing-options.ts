// Shared option data for the get-listed form (public) and the admin "Add customer"
// form. Single source of truth so the country/currency/policy lists never drift.

export const DURATIONS: [string, string][] = [['2h', '2 hours'], ['4h', '4 hours'], ['6h', '6 hours'], ['day', 'Full day']]
...
export function symbolOf(code: string): string { return CURRENCIES.find((c) => c[0] === code)?.[1] ?? code }
...
export const COUNTRIES = DIAL_CODES.map(([flag, name]) => [flag, name] as [string, string])
...
export interface BoatRow { name: string; url: string; prices: Record<string, string>; cancellation: string; cancellationCustom: string }
...
export function emptyPrices(): Record<string, string> { return { '2h': '', '4h': '', '6h': '', day: '' } }
export function newBoat(): BoatRow { return { name: '', url: '', prices: emptyPrices(), cancellation: 'moderate', cancellationCustom: '' } }
