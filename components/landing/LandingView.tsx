...
export default function LandingView({ page }: { page: LandingPage }) {
  ...
  return (
    <div style={{ background: '#07101e', color: '#f4f4f2', minHeight: '100vh' }}>
      ...
      <article style={{ maxWidth: '760px', margin: '0 auto', padding: '48px 24px 40px' }}>
        ...
        {page.faqs?.length ? (
          <section style={{ marginTop: '48px' }}>
            ...
            {page.faqs.map((f, i) => (
              <details key={i} style={{ borderBottom: '1px solid rgba(201,168,78,0.12)', padding: '14px 0' }}>
      ...
      {/* ── CTA ── */}
      <section style={{ borderTop: '1px solid rgba(201,168,78,0.10)', background: goldFaint }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <Link href="/search" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '13px 28px', borderRadius: '99px', background: 'linear-gradient(135deg,#d4b05e,#c9a84e,#b8942e)', color: '#07101e', fontWeight: 700, fontSize: '14px', textDecoration: 'none', border: `1px solid ${goldBorder}` }}>
            Browse all boats
          </Link>
        </div>
      </section>
